...
UI_SETTINGICON_INFO ui_icon_set[6];
static struct
{
...
static foc_knob_param_t setting_foc_knob_param_lst[] = {
    [0] = {100, 0, 3.3 * PI / 180, 1, 1, 1.1, ""}, // 亮度0-100
    [1] = {7, 0, 30 * PI / 180, 1, 1, 1.1, ""},    // 息屏时间
    [2] = {7, 0, 30 * PI / 180, 1, 1, 1.1, ""},    // 关机时间
    [3] = {4, 0, 30 * PI / 180, 1, 1, 1.1, ""},    // 屏幕方向
    [4] = {4, 0, 30 * PI / 180, 1, 1, 1.1, ""},    // wifi开关
    [5] = {2, 0, 30 * PI / 180, 1, 1, 1.1, ""},    // RGB开关
};
...
static void ui_icon_info_init()
{
    uint8_t id;
    id = 0;
    ui_icon_set[id].icon_id = id;
    ui_icon_set[id].param_list = setting_foc_knob_param_lst[id];

    ui_icon_set[id].icon.img_src = &ui_img_light_png;
    ui_icon_set[id].icon.name = "亮度";
    ui_icon_set[id].current_position = 100;
    ui_icon_set[id].param_list.position = ui_icon_set[id].current_position;

    id = 1;
    ui_icon_set[id].icon_id = id;
    ui_icon_set[id].param_list = setting_foc_knob_param_lst[id];

    ui_icon_set[id].icon.img_src = &ui_img_screenlocklandscape_png;
    ui_icon_set[id].icon.name = "息屏时间";
    ui_icon_set[id].current_position = 3;
    ui_icon_set[id].param_list.position = ui_icon_set[id].current_position;
    ...
    ui_icon_set[id].icon.img_src = &ui_img_sleep_png;
    ui_icon_set[id].icon.name = "关机时间";
    ui_icon_set[id].current_position = 1;
    ui_icon_set[id].param_list.position = ui_icon_set[id].current_position;
    ...
    ui_icon_set[id].icon.img_src = &ui_img_screenrotation_png;
    ui_icon_set[id].icon.name = "屏幕方向";
    ui_icon_set[id].current_position = 1;
    ui_icon_set[id].param_list.position = ui_icon_set[id].current_position;
    ...
    ui_icon_set[id].icon.img_src = &ui_img_wifilogo_png;
    ui_icon_set[id].icon.name = "wifi开关";
    ui_icon_set[id].current_position = 1;
    ui_icon_set[id].param_list.position = ui_icon_set[id].current_position;
    ...
    ui_icon_set[id].icon.img_src = &ui_img_home_page_png;
    ui_icon_set[id].icon.name = "RGB开关";
    ui_icon_set[id].current_position = 0;
    ui_icon_set[id].param_list.position = ui_icon_set[id].current_position;
}
...
void ui_set_screen_event(uint8_t state)
{
    ...
    case 0:
        ...
        case DIAL_TURN_RIGHT:
            if (ui_icon_set[icon_index].current_position < 100)
                ui_icon_set[icon_index].current_position++;
            set_screen_light(ui_icon_set[icon_index].current_position);
            set_knob_battery_level(ui_icon_set[icon_index].current_position);
            break;
        case DIAL_TURN_LEFT:
            if (ui_icon_set[icon_index].current_position > 1)
                ui_icon_set[icon_index].current_position--;
            set_screen_light(ui_icon_set[icon_index].current_position);
            set_knob_battery_level(ui_icon_set[icon_index].current_position);
            break;
    ...
    case 1:
        ...
        {
        case DIAL_TURN_RIGHT:
            if (ui_icon_set[icon_index].current_position < 6)
                ui_icon_set[icon_index].current_position++;
            // set_new_lock_time(ui_icon_set[icon_index].current_position);
            break;
        case DIAL_TURN_LEFT:
            if (ui_icon_set[icon_index].current_position > 0)
                ui_icon_set[icon_index].current_position--;
            // set_new_lock_time(ui_icon_set[icon_index].current_position);
            break;
        }
    ...
    case 3:
        ...
        {
        case DIAL_TURN_RIGHT:
            if (ui_icon_set[icon_index].current_position < 3)
                ui_icon_set[icon_index].current_position++;
            break;
        case DIAL_TURN_LEFT:
            if (ui_icon_set[icon_index].current_position > 0)
    ...
    case 5:
        ...
        case DIAL_TURN_RIGHT:
            if (ui_icon_set[icon_index].current_position < 1)
                ui_icon_set[icon_index].current_position++;
            RGB_power_set(ui_icon_set[icon_index].current_position);
            break;
        case DIAL_TURN_LEFT:
            if (ui_icon_set[icon_index].current_position > 0)
                ui_icon_set[icon_index].current_position--;
    ...
    ui_icon_set[icon_index].param_list.position = ui_icon_set[icon_index].current_position;
    switch (state)
    {
    case DIAL_PRESS_UP:
        foc_knob_user_set_param(ui_icon_set[icon_index].param_list);
        break;
    ...
    case DIAL_DOUBLE_CLICK:
        _ui_screen_change(&ui_set, &ui_main, LV_SCR_LOAD_ANIM_FADE_ON, 300, 0, &ui_main_screen_init);
        break;
...
{
    lv_label_set_text(ui_label.name, ui_icon_set[index].icon.name);
    lv_label_set_text(ui_label.mid, ui_icon_set[index].icon.mid_info);
    // 聚焦icon 显示对应lv_obj_t
    switch (index)
    {
    case 0: // 屏幕亮度
        ...
        lv_arc_set_value(ui_label.arc, ui_icon_set[index].current_position);
        lv_label_set_text_fmt(ui_label.mid, "%d", ui_icon_set[index].current_position);
        ui_icon_set[index].param_list.position = ui_icon_set[index].current_position - 1;
        break;
    case 1: // 息屏时间
        ...
        lv_roller_set_selected(ui_label.roller, ui_icon_set[index].current_position, LV_ANIM_ON);
        ui_icon_set[index].param_list.position = ui_icon_set[index].current_position;
        break;
    case 2: // 关机时间
        lv_obj_clear_flag(ui_label.roller, LV_OBJ_FLAG_HIDDEN);
        lv_obj_add_flag(ui_label.arc, LV_OBJ_FLAG_HIDDEN);
    ...
    case 4: // wifi开关
        ...
        lv_obj_add_flag(ui_label.bottom_label, LV_OBJ_FLAG_HIDDEN);
        lv_roller_set_options(ui_label.wifi_roller, "NULL\nSTA\nAP\nSTA+AP", LV_ROLLER_MODE_NORMAL);
        lv_roller_set_selected(ui_label.wifi_roller, ui_icon_set[index].current_position, LV_ANIM_ON);
        ui_icon_set[index].param_list.position = ui_icon_set[index].current_position;
        break;
    case 5: // rgb开关
        lv_obj_add_flag(ui_label.wifi_roller, LV_OBJ_FLAG_HIDDEN);
        ...
        lv_obj_clear_flag(ui_label.roller, LV_OBJ_FLAG_HIDDEN);
        lv_obj_add_flag(ui_label.mid, LV_OBJ_FLAG_HIDDEN);
        lv_roller_set_options(ui_label.roller, "OFF\nON", LV_ROLLER_MODE_NORMAL);
        lv_roller_set_selected(ui_label.roller, ui_icon_set[index].current_position, LV_ANIM_ON);
        ui_icon_set[index].param_list.position = ui_icon_set[index].current_position;
        break;
    default:
    ...
    // foc_knob_set_param(ui_icon_set[index].param_list);
}
...
static void scr_Screen_Setting_unloaded_cb(lv_event_t *e)
{
    ...
    if (set_task)
    {
        lv_timer_del(set_task);
        set_task = NULL; // 置空，避免悬空指针
    }
}
static void scr_Screen_Setting_loaded_cb(lv_event_t *e)
{
    ui_state.index = UI_SET;
...
static void task_timer_cb()
{
    ...
    {
    case 0:
        lv_arc_set_value(ui_label.arc, ui_icon_set[icon_index].current_position);
        lv_label_set_text_fmt(ui_label.mid, "%d", ui_icon_set[icon_index].current_position);
        break;
    case 1:
        lv_roller_set_selected(ui_label.roller, ui_icon_set[icon_index].current_position, LV_ANIM_ON);
        break;
    case 2:
        lv_roller_set_selected(ui_label.roller, ui_icon_set[icon_index].current_position, LV_ANIM_ON);
        break;
    case 3:
        lv_roller_set_selected(ui_label.roller, ui_icon_set[icon_index].current_position, LV_ANIM_ON);
        set_screen_rotation(ui_icon_set[icon_index].current_position);
        break;
    case 4:
        lv_roller_set_selected(ui_label.wifi_roller, ui_icon_set[icon_index].current_position, LV_ANIM_ON);
        break;
    case 5:
        lv_roller_set_selected(ui_label.roller, ui_icon_set[icon_index].current_position, LV_ANIM_ON);
        break;
...
void ui_set_screen_init(void)
{
    ...
    ui_icon_info_init();
    lv_obj_add_event_cb(ui_set, scr_Screen_Setting_loaded_cb, LV_EVENT_SCREEN_LOADED, &ui_set);
    lv_obj_add_event_cb(ui_set, scr_Screen_Setting_unloaded_cb, LV_EVENT_SCREEN_UNLOADED, &ui_set);
    lv_obj_clear_flag(ui_set, LV_OBJ_FLAG_SCROLLABLE); /// Flags
    lv_obj_set_style_bg_img_src(ui_set, &ui_img_bg1_png, LV_PART_MAIN | LV_STATE_DEFAULT);
    Style_Init();
    Create(ui_set);
    setl_label_info(icon_index);
    lv_obj_t *container = container_create(ui_set);
    for (uint8_t i = 0; i < ICON_CNT; i++)
    {
        icon_create(container, ui_icon_set[i].icon.img_src, ui_icon_set[i].img_angle);
    }
    uint32_t mid_btn_index = (lv_obj_get_child_cnt(container) - 1) / 2;
    ...
    set_task = lv_timer_create(task_timer_cb, 50, 0);
    lv_timer_set_repeat_count(set_task, -1);
}
