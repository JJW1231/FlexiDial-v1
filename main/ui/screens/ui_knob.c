#include "ui.h"
#include "driver/hid/esp_hidd_prf_api.h"
...
void ui_knob_screen_event(uint8_t state)
{
    ...
    case DIAL_PRESS_DOWN:
        ble_hid_surfacedial_report(DIAL_PRESS_DOWN);
        break;
    case DIAL_PRESS_UP:
        ble_hid_surfacedial_report(DIAL_PRESS_UP);
        break;
    case DIAL_TURN_RIGHT:
        ble_hid_surfacedial_report(DIAL_TURN_RIGHT);
        break;
    case DIAL_TURN_LEFT:
        ble_hid_surfacedial_report(DIAL_TURN_LEFT);
        break;
    case DIAL_DOUBLE_CLICK:
        _ui_screen_change(&ui_knob, &ui_main, LV_SCR_LOAD_ANIM_FADE_ON, 300, 0, &ui_main_screen_init);
        break;
    default:
        break;
    }
}
void scr_knob_unloaded_cb(lv_event_t *e)
{
...
void scr_knob_loaded_cb(lv_event_t *e)
{
...
{
    ui_knob = lv_obj_create(NULL);
    lv_obj_add_event_cb(ui_knob, scr_knob_loaded_cb, LV_EVENT_SCREEN_LOADED, &ui_knob);
    // lv_obj_add_event_cb(ui_knob, scr_knob_unloaded_cb, LV_EVENT_SCREEN_UNLOADED, &ui_knob);
    lv_obj_set_style_bg_img_src(ui_knob, &ui_img_bg_dial_png, LV_PART_MAIN | LV_STATE_DEFAULT);
    // lv_obj_clear_flag(ui_knob, LV_OBJ_FLAG_SCROLLABLE); /// Flags
    // Style_Init();
}
