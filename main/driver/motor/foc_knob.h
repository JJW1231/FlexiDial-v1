...
#define PI 3.14159265358979f
    ...
    typedef struct
    {
        int32_t num_positions;        /*!<位置数*/
        int32_t position;             /*!<当前位置 */
        float position_width_radians; /*!<以弧度为单位的每个位置的宽度 */
        float detent_strength_unit;   /*!<正常旋转时的缓冲力*/
        float endstop_strength_unit;  /*!<到达终点时的制动强度*/
        float snap_point;             /*!<位置切换点 */
        const char *descriptor;       /*!<说明*/
    } foc_knob_param_t;
