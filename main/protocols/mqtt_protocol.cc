...
bool MqttProtocol::StartMqttClient(bool report_error)
{
    ...
    ESP_LOGI(TAG, "端点:%s", endpoint_.c_str());
    ESP_LOGI(TAG, "客户端_id:%s", client_id_.c_str());
    ESP_LOGI(TAG, "用户名:%s", username_.c_str());
    ESP_LOGI(TAG, "密码:%s", password_.c_str());
    ESP_LOGI(TAG, "publish_topic:%s", publish_topic_.c_str());

    if (endpoint_.empty())
    {
        ESP_LOGW(TAG, "没有指定MQTT端点");
        if (report_error)
        {
    ...
    mqtt_->OnMessage([this](const std::string &topic, const std::string &payload)
        ...
        if (strcmp(type->valuestring, "hello") == 0) {
            ...
            ESP_LOGI(TAG, "收到再见信息, session_id: %s", session_id ? session_id->valuestring : "null");
    ...
    // 尝试连接到MQTT服务器
    ESP_LOGI(TAG, "连接到端点 %s", endpoint_.c_str());
...
void MqttProtocol::SendText(const std::string &text)
{
    ...
    {
        ESP_LOGE(TAG, "发布消息失败: %s", text.c_str());
